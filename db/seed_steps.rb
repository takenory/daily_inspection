...
def the_following_maxlength_data_are_registered(table_str)
  ...
  cuke_table_rows(table_str).each do |count, station_char, station_max, equipment_char, equipment_max, cycle_title, part_char, part_max, item_char, item_max, operation_char, operation_max, criterion_char, criterion_max, checked_date_before, scheduled_date_after, scheduled_time_after|
    count.to_i.times.each do |i|
      station = Station.find_or_create_by_name(station_max.to_i.times.collect{station_char}.join)
      station = Station.create(:name => station_max.to_i.times.collect{station_char}.join)
      equipment = Equipment.find_or_create_by_station_id_and_name(station.id, equipment_max.to_i.times.collect{equipment_char}.join)
      part = Part.find_or_create_by_equipment_id_and_sequence_and_name(equipment.id, i, part_max.to_i.times.collect{part_char}.join)
      ...
      check_master = Check.create!(:cycle_id => cycle.id, :part_id => part.id, 
                                   :item => item_max.to_i.times.collect{item_char}.join, :sequence => i, :entry_type => 0, 
                                   :operation => operation_max.to_i.times.collect{operation_char}.join, 
                                   :criterion => criterion_max.to_i.times.collect{criterion_char}.join, 

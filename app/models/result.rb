...
class Result < ActiveRecord::Base
  ...
  attr_accessible :check_id, :judgment, :measured_value, :abnormal_content, :staff_code, :corrected_datetime, :corrective_action, :corrective_content, :checked_datetime

  validates_length_of :abnormal_content, :maximum => 140, :allow_nil => true
  validate :entry_type_and_judgement
  ...
  # ＜validationせずに保存する場合＞
  # ・値はそのままにする
  def save_with_validation(with_validation = true)
    if with_validation
    ...
    self.save(:validate => with_validation)
  end
  ...
  def update_correction(attr)
    ...
    unless attr[:corrected_datetime] =~ /\A\d{4}\/\d{1,2}\/\d{1,2} \d{1,2}:\d{1,2}:\d{1,2}\z/
      errors.add :base, I18n.t("activerecord.errors.models.result.corrected_date_format_was_invalid") 
    else
      errors.add :base, I18n.t("activerecord.errors.models.result.corrected_date_format_was_invalid") if ((Time.parse(attr[:corrected_datetime]) rescue ArgumentError) == ArgumentError)
    end

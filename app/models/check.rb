...
class Check < ActiveRecord::Base
  ...
  attr_accessible :id, :inspection_id, :part_id, :item, :operation, :criterion, :unit_name, :sequence, :created_at, :updated_at, :parent_id, :cycle_id, :cycle_title, :part_name,
                  :entry_type, :scheduled_datetime, :upper_limit, :lower_limit
  ...
  # 次の点検内容
  def next_check
    Check.where('inspection_id = ? and sequence > ?', self.inspection_id, self.sequence)
         .order('sequence ASC')
         .first
  end
  ...
  def save_result_with_validation(result_attributes, with_validation = true)
    if self.result
      ...
      self.build_result(result_attributes) 
    end
    self.result.save(:validate => with_validation)
  end
  ...
  def update_master_schedule!
    master = Check.includes(:cycle)
                  .where('checks.id = ?', self.parent_id)
                  .first
  ...
  def update_scheduled_datetime
    ...
    when Cycle::UNIT_TYPES["MONTHS"]
      ...
      errors.add :base, :cannot_update_scheduled_datetime_because_cycle_was_invalid
      return false
    end
  end
  ...
  def latest_check
    Check.includes(:inspection)
         .where('checks.parent_id = ? and inspections.staff_code is NOT NULL', self.id)
  ...
  def make_child(sequence)
    check = Check.new(self.attributes.merge(:id => nil))
  ...
  def result_list_within_the_period(start_of_period, next_start_of_period)
    ...
    Check.includes(:inspection, :result)
         .where('parent_id = ? AND inspections.staff_code is not null AND inspections.updated_at >= ? AND inspections.updated_at < ?', self.id, start_of_period, next_start_of_period)
         .order('inspections.updated_at ASC').all.each do |check|
  ...
  def self.find_with_inspection_and_result_and_part(check_id)
    check = Check.includes(:inspection, :result, :part)
                 .where('checks.id = ? AND checks.parent_id is not null AND inspections.staff_code is not null', check_id)
                 .first

...
describe EquipmentController do
  ...
  def create_station_and_equipment_and_parts_and_checks(parts_count, checks_count_by_part)
    station = FactoryGirl.create(:station_saved_in_session)
    @equipment = FactoryGirl.create(:equipment_requested_from_web_api, :station_id => station.id)
    @parts = parts_count.times.collect do |i|
      FactoryGirl.create(:part_in_equipment_requested_from_web_api, :equipment_id => @equipment.id, :name => "part_#{i}", :sequence => i)
    end
    @checks = @parts.collect do |part|
      checks_count_by_part.times.collect do |i|
        FactoryGirl.create(:check_scheduled_at_1_hour_before, :part_id => part.id, :sequence => i)
      end
    end
    @checks.flatten!
  end
  ...
  describe "'show_list_of_result/:id'アクションのテスト" do
    context "点検が存在する場合" do
      ...
      it "equipment/show_list_of_resultテンプレートが、レイアウト'pc'を用いてレンダリングされること" do
        ...
        response.should render_template("layouts/pc")
  ...
  describe "'start_inspection/:id'アクションのテスト" do
    before(:all) do
      cycle = FactoryGirl.create(:cycle_of_shift_for_start_inspection_in_controllers)
      @equipment_id = FactoryGirl.create(:equipment_for_start_inspection_in_controllers).id
      @part_count, @check_count = 2, 2
      @check_masters = []
      @now = DateTime.now
      @part_count.times.each do |part_seq|
        part = FactoryGirl.create(:part_for_start_inspection_in_controllers,
                              :name => "部位_#{part_seq}",
                              :sequence => part_seq,
                              :equipment_id => @equipment_id)
        @check_count.times.each do |check_seq|
          @check_masters << FactoryGirl.create(:check_of_master_for_start_inspection_in_controllers,
                                           :part_id => part.id,
                                           :cycle_id => cycle.id,
                                           :sequence => check_seq,
    ...
    context "点検が開始できる場合" do
      ...
      it "点検画面にリダイレクトされること" do
        check_id = Check.where('parent_id = ?', @check_masters[0].id).first.id
        response.should be_redirect

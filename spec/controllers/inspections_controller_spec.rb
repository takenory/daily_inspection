...
describe InspectionsController do
  ...
  describe "'final_confirm/:id'アクションのテスト" do
    context "inspectionデータを表示する場合" do
      before do
        @inspection = FactoryGirl.create(:inspection_has_check)
        @check = FactoryGirl.create(:check_belong_to_inspection, :inspection_id => @inspection.id)
  ...
  describe "updateアクションのテスト" do
    ...
    before do
      check = FactoryGirl.create(:check_has_valid_result)
      @inspection_for_result = FactoryGirl.create(:inspection_for_update_action)
      @inspection_for_result.checks[0] =
        FactoryGirl.create(:check_is_normal, :parent_id => check.id, :inspection_id => @inspection_for_result.id)
    ...
    context "適切な点検結果・適切な氏名コードの場合" do
      before do
        @station_id = session[:station_id] = FactoryGirl.create(:station_for_inspection_update_action).id
        @staff_code = "staff01"

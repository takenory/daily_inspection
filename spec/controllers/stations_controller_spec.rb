...
describe StationsController do
  ...
  describe "operator_indexとladder_indexアクションのテスト" do
    before(:all) do
      ...
      @stations = @stations_count.times.collect {FactoryGirl.create(:station_data_for_index)}
      @station = @stations.last
      @equipment_count.times.each {FactoryGirl.create(:equipment_data_for_index, :station_id => @station.id)}
    end

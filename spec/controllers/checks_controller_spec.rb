...
describe ChecksController do
  ...
  describe "'show/:id'アクションのテスト" do
    ...
    context "resultデータがある場合" do
      before do
        @check = FactoryGirl.create(:check_has_result_in_controller)
    ...
    context "resultデータがない場合" do
      before do
        @check = FactoryGirl.create(:check_does_not_have_result_in_controller)
        get 'show', :id => @check.id
      end
  ...
  describe "「次へ」「前へ」の遷移テスト" do
    def create_sequential_checks
      ...
      @sequential_checks = {:first => FactoryGirl.create(:check_of_sequentially_in_controller,
                                                     :inspection_id => inspection_id),
                            :middle => FactoryGirl.create(:check_of_sequentially_in_controller,
                                                      :inspection_id => inspection_id),
                            :last => FactoryGirl.create(:check_of_sequentially_in_controller,
                                                    :inspection_id => inspection_id)}
      # 点検結果を格納したリクエストパラメータ（Hash）
      @result = FactoryGirl.attributes_for(:result_of_request_parameter)
    end
    ...
    share_examples_for "check_result_should_be_saved" do
      it "@check.resultの値がリクエストパラメータ':result'の値で更新されていること" do
        ...
        check_result.measured_value.to_s.should == @result[:measured_value].to_s
  ...
  describe "適切でない点検結果の入力テスト" do
    ...
    before(:all) do
      # テストデータの作成
      inspection_id = Check.maximum(:inspection_id).to_i + 1
      @two_checks = {:first => FactoryGirl.create(:check_of_sequentially_in_controller,
                                              :inspection_id => inspection_id),
                     :last => FactoryGirl.create(:check_of_sequentially_in_controller,
                                             :inspection_id => inspection_id)}
      @invalid_request = FactoryGirl.attributes_for(:result_of_invalid_request_paramter)
    end
    ...
    context "'next/:id'アクション(次への遷移)の場合" do
      before do
        ...
        seq_check.result ||= FactoryGirl.build(:result_init_instance)
    ...
    context "'previous/:id'アクション(前への遷移)の場合" do
      before do
        @two_checks[:last].result ||= FactoryGirl.build(:result_init_instance)

...
describe CorrectionsController do
  ...
  describe "'show/:id'アクションのテスト" do
    before do
      @check = FactoryGirl.create(:check_has_result_and_inspection)
    ...
    it { response.should render_template("layouts/pc") }
  ...
  describe "updateアクションのテスト" do
    context "正常に更新できる場合" do
      ...
      it "更新が成功すること" do
        ...
        result.corrected_datetime.to_s.should == "2010-12-15 12:34:56 UTC"
      end
    ...
    context "正常に更新できない場合" do
      ...
      it "異常対応確認入力画面に遷移されること" do
        flash[:message].should == assigns[:check].result.errors.full_messages.join('\n')

...
describe Equipment do
  describe "partsテーブルとの関連付けテスト" do
    ...
    def create_equipment_and_related_part(parts_count)
      @equipment = FactoryGirl.create(:equipment_has_parts)
      parts_count.times.collect {|i| FactoryGirl.create(:part_belongs_to_equipment, :sequence => i, :equipment_id => @equipment.id)}
    end
  ...
  describe "任意の日時における'設備'毎の点検データを抽出するテスト" do
    ...
    def create_check_at_each_equipment_and_scheduled_at(datetime)
      ...
      @equipment = FactoryGirl.create(:equipment_has_checks_throw_part)
      @parts = @parts_count.times.collect {|i| FactoryGirl.create(:part_at_each_equipment, :sequence => i, :equipment_id => @equipment.id)}
      @parts.each do |part|
        @checks_count.times.collect do |i|
          FactoryGirl.create(:check_at_each_equipment_scheduled_at, :part_id => part.id, :scheduled_datetime => datetime, :sequence => i)
        end
      end
    end
  ...
  describe "点検開始時のテスト" do
    before(:all) do
      cycle = FactoryGirl.create(:cycle_of_shift_for_start_inspection)
      @equipment_id = FactoryGirl.create(:equipment_for_start_inspection).id
      @part_count, @check_count = 2, 2
      @check_masters = []
      @now = DateTime.now
      @part_count.times.each do |part_seq|
        part = FactoryGirl.create(:part_for_start_inspection,
          :name => "部位_#{part_seq}",
          :sequence => part_seq,
          :equipment_id => @equipment_id)
        @check_count.times.each do |check_seq|
          @check_masters << FactoryGirl.create(:check_of_master_for_start_inspection,
            :part_id => part.id,
            :cycle_id => cycle.id,
            :sequence => check_seq,
    ...
    context "点検開始できる場合" do
      it "点検マスタデータから点検項目データを作成できること" do
        ...
        check = Check.where('parent_id = ?', @check_masters[0].id).first
        first_check_id.should == check.id
        checks = Check.where('inspection_id = ?', check.inspection_id)
                      .order('sequence ASC')
                      .all
  ...
  def create_equipment_and_part_and_check_of_parent_included_cycle(parts_count, check_masters_count)
    ...
    @equipment = FactoryGirl.create(:equipment_for_check_of_parents_included_part_and_cycle) 
    parts = parts_count.times.collect do |i|
      FactoryGirl.create(:part_for_check_of_parents_included_part_and_cycle, :equipment_id => @equipment.id, :sequence => i)
    end
    @check_masters = parts.collect do |part|
      check_masters_count.times.collect do |i|
        FactoryGirl.create(:check_for_check_of_parents_included_part_and_cycle, :part_id => part.id, :parent_id => nil, :sequence => i)
      end
    end.flatten!
  end
    ...
    context "設備に紐付く点検内容がある場合" do
      before do
        ...
        @check_masters.each do |master|
          5.times.each do |i|
            inspection = FactoryGirl.create(:inspection_for_check_of_parents_included_part_and_cycle, 
                                        :updated_at => @datetime - Rational(i + 1, 24))
            check = FactoryGirl.create(:check_for_check_of_parents_included_part_and_cycle, 
                                   :inspection_id => inspection.id,
                                   :part_id => master.part.id, 
                                   :parent_id => master.id, 
                                   :sequence => master.sequence)
            result = FactoryGirl.create(:result_for_check_of_parents_included_part_and_cycle, 
                                    :check_id => check.id)
          end
        end
      end

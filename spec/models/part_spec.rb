...
describe Part do
  describe "checksテーブルとの関連付けテスト" do
    ...
    def create_parts_and_related_checks(check_count)
      @part = FactoryGirl.create(:part_has_checks)
      @checks = check_count.times.collect {|i| FactoryGirl.create(:check_belongs_to_part, :sequence => i, :part_id => @part.id)}
    end
  ...
  describe "任意の日時における'部位'の点検データを抽出するテスト" do
    ...
    def create_part_and_scheduled_checks_at(datetime)
      @checks_count = 2
      @part = FactoryGirl.create(:part_has_checks_spacified_scheduled_datetime)
      @checks = @checks_count.times.collect do |i|
          FactoryGirl.create(:check_at_each_part_scheduled_at,
                         :part_id => @part.id, 
                         :sequence => i,

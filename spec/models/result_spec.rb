...
describe Result do
  ...
  describe "checksテーブルとの関連付けのテスト" do
    context "'checks'テーブルにレコードが存在する場合" do
      subject { FactoryGirl.create(:result_has_check) }
  ...
  describe "点検基準が数値である場合に入力された点検結果(数値)から正常/異常を判断するテスト" do
    ...
    context "入力された点検結果が下限値より小さい場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_smaller_than_lower_limit)
      end
    ...
    context "入力された点検結果が下限値と等しい場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_equal_lower_limit)
      end
    ...
    context "入力された点検結果が下限値より大きく、上限値が設定されていない場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_larger_than_lower_limit_withiout_upper_limit)
      end
      it_should_behave_like "result_judgment_should_be_true"
    end
    ...
    context "入力された点検結果が下限値より大きく、上限値より小さい場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_in_range)
      end
      it_should_behave_like "result_judgment_should_be_true"
    end
    ...
    context "入力された点検結果が上限値より小さく、下限値が設定されていない場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_smaller_than_upper_limit_without_lower_limit)
      end
      it_should_behave_like "result_judgment_should_be_true"
    end
    ...
    context "入力された点検結果が上限値と等しい場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_equal_upper_limit)
      end
      it_should_behave_like "result_judgment_should_be_true"
    end
    ...
    context "入力された点検結果が上限値より大きい場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_larger_than_upper_limit)
      end
      it_should_behave_like "result_judgment_should_be_false"
    end
    ...
    context "点検結果が未入力の場合" do
      before do
        @result = FactoryGirl.build(:result_measured_value_is_nil)
      end
  ...
  describe "点検結果、異常内容、対応・処置が正しく入力/選択されているかvalidationでチェックするテスト" do
    ...
    describe "点検結果で正常を選択した場合のテスト" do
      ...
      context "異常内容に入力した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_true_with_abnormal_content)
        end
      ...
      context "対応・処置のいずれかを選択した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_true_with_corrective_action)
        end
    ...
    describe "点検結果で異常を選択した場合のテスト" do
      context "入力された異常内容がマルチバイトで140字以内の場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_false_with_abnormal_content_under_140chars_multibyte)
        end
        it "保存が成功すること" do
          @check.save_result_with_validation(@result_data).should be_true
          @check.result.judgment.should == @result_data[:judgment]
      ...
      context "入力された異常内容がマルチバイトで141字以上の場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_false_with_abnormal_content_over_140chars_multibyte)
        end
        it "保存が失敗すること" do
      ...
      context "入力された異常内容がシングルバイトで140字以内の場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_false_with_abnormal_content_under_140chars_singlebyte)
        end
        it "保存が成功すること" do
          @check.save_result_with_validation(@result_data).should be_true
          @check.result.abnormal_content.should == @result_data[:abnormal_content]
      ...
      context "入力された異常内容がシングルバイトで141字以上の場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_false_with_abnormal_content_over_140chars_singlebyte)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
      ...
      context "異常内容が入力されていない場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_selected_false_without_abnormal_content)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
    ...
    describe "点検結果で正常/異常の選択に不備がある場合のテスト" do
      context "正常/異常を選択していない場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_0)
          @result_data = FactoryGirl.attributes_for(:result_is_not_selected_judgment)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
    ...
    describe "点検結果で入力した測定値が基準範囲内(正常)である場合のテスト" do
      context "異常内容に入力した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_true_with_abnormal_content)
        end
        it "保存が成功し、異常内容は保存されないこと" do
          @check.save_result_with_validation(@result_data).should be_true
      ...
      context "対応・処置のいずれかを選択した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_true_with_corrective_action)
        end
        it "保存が成功し、対応・処置は保存されないこと" do
          @check.save_result_with_validation(@result_data).should be_true
          @check.result.judgment.should be_true
      ...
      context "既にあるResultsテーブルに対して基準範囲内(正常)で更新した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_true)
          @check.save_result_with_validation(@result_data)
          @check.reload
        end
        it "更新が成功すること" do
          @result_update_data = FactoryGirl.attributes_for(:result_is_enterd_true_for_update)
      ...
      context "測定値の数値に小数点以下を５桁入力した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_true_with_measured_value_over_digit)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
      ...
      context "測定値に整数部分が４桁の負の値を入力した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_dummy)
    ...
    describe "点検結果で入力した測定値が基準範囲外(異常)である場合のテスト" do
      context "入力された異常内容が140字以内の場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_false_with_abnormal_content_under_140chars_multibyte)
        end
        it "保存が成功すること" do
          @check.save_result_with_validation(@result_data).should be_true
      ...
      context "入力された異常内容が141字以上の場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_false_with_abnormal_content_over_140chars_singlebyte)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
      ...
      context "異常内容が入力されていない場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_false_without_abnormal_content)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
      ...
      context "既にあるResultsテーブルに対して基準範囲外(正常)かつ異常内容を入力せずに更新した場合" do
        ...
        it "更新が失敗すること" do
          @result_update_data = FactoryGirl.attributes_for(:result_is_enterd_false_without_abnormal_content)
    ...
    describe "点検結果で測定値の入力に不備がある場合のテスト" do
      context "測定値を入力していない場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_not_enterd_measured_value)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
      ...
      context "測定値に数値以外を入力した場合" do
        before do
          @check = FactoryGirl.create(:check_has_result_with_entry_type_1)
          @result_data = FactoryGirl.attributes_for(:result_is_enterd_character_into_measured_value)
        end
        it "保存が失敗すること" do
          @check.save_result_with_validation(@result_data).should be_false
        end
  ...
  describe "対応者氏名コードを更新するテスト" do
    before do
      @inspection = FactoryGirl.create(:inspection_has_normal_check_for_set_staff_code)
      @check_master = FactoryGirl.create(:check_has_normal_cycle_for_set_staff_code)
      @inspection.checks[0] = FactoryGirl.create(:check_is_normal_for_set_staff_code,:parent_id => @check_master.id, :inspection_id => @inspection.id)
    end
    context "対応日時が入っている場合" do
      before do
        @inspection.checks[0].save_result_with_validation({:judgment => false, :abnormal_content => "異常です。", :corrective_action => 0, :corrected_datetime => DateTime.now}, false)
    ...
    context "対応日時は入っているが例外が発生する場合" do
      before do
        @inspection.checks[0].save_result_with_validation({:judgment => false, :corrective_action => 0, :corrected_datetime => DateTime.now}, false)
        @check = @inspection.checks[0]
        @result_updated_at = @check.result.updated_at
      end
    ...
    context "対応日時が入っていない場合" do
      before do
        @inspection.checks[0].save_result_with_validation({:judgment => true}, false)
        @check = @inspection.checks[0]
        @result_updated_at = @check.result.updated_at
      end
      it "対応者氏名コードが更新されないこと" do
  ...
  describe "点検日時と対応日時の値を保存するテスト" do
    before do
      @result = FactoryGirl.create(:check_has_result_with_checked_datetime_and_corrected_datetime).result
    ...
    share_examples_for "checked_datetime_and_corrected_datetime_should_be_updated_with_validation" do
      it "点検日時と対応日時が更新されていること" do
        ...
        @result.checked_datetime.to_s.should == @result.updated_at.to_s
        @result.corrected_datetime.to_s.should == @result.updated_at.to_s
      end
    end
  ...
  describe "'対応内容'を更新する際の入力チェック機能" do
    before do
      @result = FactoryGirl.create(:result_for_validation_check_before_update_correction)
    ...
    context "パラメータが正しくセットされた場合" do
      ...
      it "'results'テーブルのレコードが更新されること" do
        ...
        result.corrected_datetime.to_s.should ==  "2010-12-13 12:34:56 UTC"

...
describe Inspection do
  describe "点検結果（スタッフコード）の保存テスト" do
    context "点検結果に不備がない場合" do
      before do
        @inspection_for_valid_result_data = FactoryGirl.create(:inspection_valid_result_data)
        Check.destroy_all(:inspection_id => @inspection_for_valid_result_data.id)
        check = FactoryGirl.create(:check_has_valid_result_judgment_false)
    ...
    context "点検結果に不備がある場合" do
      it "適切でない点検結果データ保存されている場合は、スタッフコードを保存できないこと" do
        check = FactoryGirl.create(:check_has_invalid_result)
...
#        inspection.errors.full_messages.should include("1" + I18n.t("activerecord.errors.models.inspection.check_result_was_invalid"))
        inspection.errors.full_messages.should include(I18n.t("activerecord.errors.models.inspection.check_result_was_invalid", :num => 1))
      end

      it "未点検の点検内容がある場合は、スタッフコードを保存できないこと" do
        inspection = Inspection.find(FactoryGirl.create(:check_has_not_been_checked).inspection_id)
        inspection.update_attributes(:staff_code => "staff01").should be_false
#        inspection.errors.full_messages.should include("1" + I18n.t("activerecord.errors.models.inspection.has_not_been_checked"))
        inspection.errors.full_messages.should include(I18n.t("activerecord.errors.models.inspection.has_not_been_checked", :num => 1))
      end
    end
  end
  ...
  describe "checksテーブルとの関連付けのテスト" do
    context "'checks'テーブルにレコードが存在する場合" do
      before do
        @inspection = FactoryGirl.create(:inspection_has_checks)
      end
    ...
    context "'checks'テーブルにレコードが存在しない場合" do
      before do
        @inspection = FactoryGirl.create(:inspection_does_not_have_check)
      end
  ...
  describe "点検結果を保存するテスト" do
    context "スタッフコード及び点検予定日を更新できる場合" do
      before do
        @check = FactoryGirl.create(:check_has_normal_cycle)
        @check_updated_at = @check.updated_at
        @inspection = FactoryGirl.create(:inspection_has_normal_checks)
        @inspection_update_at = @inspection.updated_at

        @inspection.checks[0] = FactoryGirl.create(:check_is_normal1, :parent_id => @check.id, :inspection_id => @inspection.id)
        @inspection.checks[0].save_result_with_validation({:judgment => true})
        @check1_updated_at = @inspection.checks[0].updated_at

        @inspection.checks[1] = FactoryGirl.create(:check_is_normal2, :parent_id => @check.id, :inspection_id => @inspection.id)
    ...
    context "スタッフコードを更新できない場合" do
      ...
      it "更新日時が更新されていないこと" do
        ...
        @inspection.reload
        @inspection.updated_at.to_s.should == @inspection_update_at.to_s
        @check.reload
        @check.updated_at.to_s.should == @check_updated_at.to_s
      end
    end

    context "点検予定日を更新できない場合" do
      before do
        @check = FactoryGirl.create(:check_has_abnormal_cycle)
        @check_updated_at = @check.updated_at
        @inspection = FactoryGirl.create(:inspection_has_abnormal_checks)
        @inspection_update_at = @inspection.updated_at

        @inspection.checks[0] = FactoryGirl.create(:check_is_abnormal1, :parent_id => @check.id, :inspection_id => @inspection.id)
        @inspection.checks[0].save_result_with_validation({:judgment => true})
        @check1_updated_at = @inspection.checks[0].updated_at

        @inspection.checks[1] = FactoryGirl.create(:check_is_abnormal2, :parent_id => @check.id, :inspection_id => @inspection.id)
        @inspection.checks[1].save_result_with_validation({:judgment => true})
        @check2_updated_at = @inspection.checks[1].updated_at
      end
      it "更新日時が更新されていないこと" do

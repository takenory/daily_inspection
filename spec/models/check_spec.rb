...
describe Check do
  ...
  describe "partsテーブルとの関連付けのテスト" do
    context "'parts'テーブルにレコードが存在する場合" do
      subject { FactoryGirl.create(:check_has_part) }
      its(:part) { should be_instance_of Part }
    end
    context "'parts'テーブルにレコードが存在しない場合" do
      subject { FactoryGirl.create(:check_does_not_have_part) }
  ...
  describe "resultsテーブルとの関連付けのテスト" do
    context "'results'テーブルにレコードが存在する場合" do
      subject { FactoryGirl.create(:check_has_result) }
      its(:result) { should be_instance_of Result }
    end
    context "'results'テーブルにレコードが存在しない場合" do
      subject { FactoryGirl.create(:check_does_not_have_result) }
  ...
  describe "連続する点検内容の振る舞いテスト" do
    before(:all) do
      ...
      @sequential_checks = {:first => FactoryGirl.create(:check_of_sequentially,
          :inspection_id => inspection_id),
        :middle => FactoryGirl.create(:check_of_sequentially,
          :inspection_id => inspection_id),
        :last => FactoryGirl.create(:check_of_sequentially,
          :inspection_id => inspection_id)}
    end
  ...
  describe "点検基準値を表現する文字列を取得する'limit_description'のテスト" do
    ...
    context '上限値と下限値が設定されている場合' do
      subject {FactoryGirl.create(:check_specified_limit,
  ...
  describe "resultsテーブルへの保存テスト" do
    ...
    context "'results'テーブルにレコードが存在する場合" do
      before do
        @check = FactoryGirl.create(:check_has_result_for_save)
      end
    ...
    context "'results'テーブルにレコードが存在しない場合" do
      before do
        @check = FactoryGirl.create(:check_does_not_have_result_for_save)
      end
      it_should_behave_like "result_should_be_saved"
    end
  end
  ...
  describe "cyclesテーブルとの関連付けのテスト" do
    context "'cycles'テーブルに関連付けられるレコードが存在する場合" do
      subject { FactoryGirl.create(:check_has_cycle) }
      its(:cycle) { should be_instance_of Cycle }
    end
    context "'cycles'テーブルに関連付けられるレコードが存在しない場合" do
      subject { FactoryGirl.create(:check_does_not_have_cycle) }
  ...
  describe "次回点検日時に'時間'を追加するテスト" do
    before do
      @hour_intaval = 4
      @check_by_shift = FactoryGirl.create(:check_scheduled_by_hour,
  ...
  describe "次回点検日時に'月'を追加するテスト" do
    before do
      @month_intaval = 1
      @check_by_monthly = FactoryGirl.create(:check_scheduled_by_month,
        :scheduled_datetime => "2010-10-18 12:00:00")
  ...
  describe "次回点検日時を周期に応じて更新するテスト" do
    def create_cycle_and_check(unit_type)
      cycle = FactoryGirl.create(:cycle_only_title,
        :unit_type => unit_type,
        :interval => 1)
      @check = FactoryGirl.create(:check_scheduled_by_cycle,
        :cycle_id => cycle.id,
        :scheduled_datetime => "2010-10-18 12:00:00")
    end
    ...
    context "点検の周期が'時間'による周期の場合" do
      ...
      it "次回点検日時が周期で指定された時間だけ更新されていること" do
        ...
        @check.scheduled_datetime.to_s.should == "2010-10-18 13:00:00 UTC"
      end
    end
    ...
    context "点検の周期が'月'による周期の場合" do
      ...
      it "次回点検日時が周期で指定された月だけ更新されていること" do
        @check.update_scheduled_datetime.should be_true
        @check.scheduled_datetime.to_s.should == "2010-11-18 12:00:00 UTC"
      end
    end
  ...
  describe "点検のマスタデータの次回点検日時を更新するテスト" do
    context "点検のマスタデータが正常な場合" do
      before do
        @cycle = FactoryGirl.create(:cycle_of_master, :unit_type => Cycle::UNIT_TYPES["HOURS"], :interval => 4)
        @master = FactoryGirl.create(:check_of_master, :cycle_id => @cycle.id, :scheduled_datetime => "2010-10-13 12:00:00")
        @check = FactoryGirl.create(:check_of_operation, :parent_id => @master.id, :scheduled_datetime => "2010-10-13 12:00:00")
      end
      it "マスタの次回点検日時が更新ができること" do
        lambda{@check.update_master_schedule!}.should_not raise_error(ActiveRecord::RecordInvalid)
        @master.reload
        @master.scheduled_datetime.to_s.should == "2010-10-13 16:00:00 UTC"
      end
    end
    context "点検のマスタデータが異常な場合" do
      before do
        @cycle = FactoryGirl.create(:cycle_of_master, :unit_type => nil)
        @master = FactoryGirl.create(:check_of_master, :cycle_id => @cycle.id, :scheduled_datetime => "2010-10-13 12:00:00")
        @check = FactoryGirl.create(:check_of_operation, :parent_id => @master.id, :scheduled_datetime => "2010-10-13 12:00:00")
      end
      it "更新メソッドで例外し、点検日時が更新されないこと" do
        ...
        @master.reload
        @master.scheduled_datetime.to_s.should == "2010-10-13 12:00:00 UTC"
      end
    end
  end
  ...
  describe "inspectionsテーブルとの関連付けのテスト" do
    context "'inspections'テーブルに関連付けられるレコードが存在する場合" do
      subject { FactoryGirl.create(:check_has_inspection) }
      its(:inspection) { should be_instance_of Inspection }
    end
    context "'inspections'テーブルに関連付けられるレコードが存在しない場合" do
      subject { FactoryGirl.create(:check_does_not_have_inspection) }
  ...
  describe "直近の点検結果を取得するテスト" do
    context "既存点検の場合" do
      before do
        @master = FactoryGirl.create(:check_of_master_for_getting_latest_result)
        ...
        @scheduled_datetimes.each_with_index do |scheduled_datetime, i|
          inspections[i] = FactoryGirl.create(:inspection_for_getting_latest_result,
            :staff_code => "staff#{i}")
          @checks[i] = FactoryGirl.create(:check_of_operation_for_getting_latest_result,
            :inspection_id => inspections[i].id,
            :parent_id => @master.id,
            :scheduled_datetime => scheduled_datetime)
          @results[i] = FactoryGirl.create(:result_for_getting_latest_result,
    ...
    context "点検未実施の場合" do
      before do
        @master = FactoryGirl.create(:check_of_master_for_getting_latest_result)
        inspection = FactoryGirl.create(:inspection_for_getting_latest_result)
        @check = FactoryGirl.create(:check_of_operation_for_getting_latest_result,
    ...
    context "直近の点検が未完了の場合" do
      before do
        @master = FactoryGirl.create(:check_of_master_for_getting_latest_result)
        inspection_1st = FactoryGirl.create(:inspection_for_getting_latest_result,
          :staff_code => "staff01")
        inspection_2nd = FactoryGirl.create(:inspection_for_getting_latest_result)
        @check_1st = FactoryGirl.create(:check_of_operation_for_getting_latest_result,
          :inspection_id => inspection_1st.id,
          :parent_id => @master.id,
          :scheduled_datetime => "2010-10-08 00:00:00")
        @check_2nd = FactoryGirl.create(:check_of_operation_for_getting_latest_result,
          :inspection_id => inspection_2nd.id,
          :parent_id => @master.id,
          :scheduled_datetime => "2010-10-15 00:00:00")
        @result_1st = FactoryGirl.create(:result_for_getting_latest_result,
          :check_id => @check_1st.id,
          :judgment => true)
        @result_2nd = FactoryGirl.create(:result_for_getting_latest_result,
  ...
  describe "点検項目をマスタから作成するテスト" do
    before do
      @cycle = FactoryGirl.create(:cycle_for_making_check_child)
      @part = FactoryGirl.create(:part_for_making_check_child,
        :sequence => 3)
      @inspection = FactoryGirl.create(:inspection_for_making_check_child)
      @master = FactoryGirl.create(:check_for_making_check_child,
        :part_id => @part.id,
        :cycle_id => @cycle.id,
        :sequence => 0,
  ...
  describe "マスタデータから指定期間内の点検結果一覧を取得するテスト" do
    ...
    def create_child_and_result(inspection_id, index)
      ...
      check.save!
      @checks[index] << check
      FactoryGirl.create(:result_for_rusult_list_within_the_period,
        :check_id => check.id)
    end
    before(:all) do
      ...
      @checks.size.times do |i|
        @masters << FactoryGirl.create(:check_for_result_list_within_the_period,
          :sequence => i)
      end
      @inspections = 27.times.collect do |i|
        FactoryGirl.create(:inspection_for_result_list_within_the_period,
  ...
  describe "点検項目データと紐付く点検データと点検結果データと部位データを取得するテスト" do
    def create_check_with_inspection_and_result_and_part(params = {:with_result => false})
      @check = FactoryGirl.create(:check_master_for_find_with_inspection_and_result_and_part).make_child(0)
      return unless params[:with_result]
      @check.save_result_with_validation({:judgment => false}, false)
      @check.inspection = FactoryGirl.create(:inspection_for_find_with_inspection_and_result_and_part, :staff_code => "1234567")
      @check.save!
    end
    ...
    context "点検マスタデータの場合" do
      before do
        @check_master = FactoryGirl.create(:check_master_for_find_with_inspection_and_result_and_part)
      end
      it "点検項目データが取得できないこと" do
